...
double DP(int a, int b) {
	if (b == (a + 1) % p.n) return dp[a][b] = 0;
	if (vis[a][b]) return dp[a][b];
	vis[a][b] = 1;

	dp[a][b] = 1e100;
	for (int i = (a + 1) % p.n; i != b; i = (i + 1) % p.n) {
		if (!(dia[a][i] && dia[i][b])) continue;
		double now = p.xmult(p.p[a], p.p[i], p.p[b]);
		now = ABS(now);
		double A = DP(a, i);
		double B = DP(i, b);
		dp[a][b] = MIN(dp[a][b], MAX(A, MAX(B, now)));
	}
	return dp[a][b];
}
...
double solve() {
	...
	for (int i = 0; i < p.n; i++) {
		for (int j = 0; j < p.n; j++) {
			dia[i][j] = p.isdiagonal(i, j);
		}
	}
	for (int i = 0; i < p.n; i++) {
		dia[i][(i + 1) % p.n] = dia[(i + 1) % p.n][i] = 1;
	}
